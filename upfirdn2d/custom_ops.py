...
try:
    from .ops import upfirdn2d as upfirdn2d_plugin
    _cached_plugins = dict({'upfirdn2d_plugin': upfirdn2d_plugin})
except ImportError:
...
def get_plugin(module_name, sources, **build_kwargs):
    ...
    try:  # pylint: disable=too-many-nested-blocks
        ...
        if len(source_dirs_set) == 1 and ('TORCH_EXTENSIONS_DIR'
                                          in os.environ):
            all_source_files = sorted(
                list(x for x in Path(list(source_dirs_set)[0]).iterdir()
                     if x.is_file()))
